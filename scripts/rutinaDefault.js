...
favo.addEventListener("click", function() {
  const favoritas = JSON.parse(localStorage.getItem(user + 'favoritas'));
  
  if (!favoritas.includes(idRutina)) {
    favoritas.push(idRutina);
    favoritas.sort();
    localStorage.setItem(user + 'favoritas', JSON.stringify(favoritas));
    path.setAttribute('d', 'M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z');

    $(favo).popover({
      content: 'Añadido a mis rutinas',
      placement: 'top',
      trigger: 'focus'
    });
    ...
    $(favo).popover({
      content: 'Eliminado de mis rutinas',
      placement: 'top',
      trigger: 'focus'
    });
